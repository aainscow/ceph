...
public:
 ...
 public:
  ...
  void exec(const ClsMethod<Tag, ClassID>& method, const ceph::buffer::list& inbl, Args&&... args) {
    ...
    exec_impl(method.cls.data(), method.name.data(), inbl, std::forward<Args>(args)...);
  }
...
public:
  ...
  ReadOp& exec(const ClsMethod<Tag, ClassID>& method, Args&&... args) & {
    ...
    Op::exec_impl(method.cls.data(), method.name.data(), std::forward<Args>(args)...);
    return *this;
  }
...
public:
  ...
  decltype(auto) exec(const ClsMethod<Tag, ClassID>& method, const ceph::buffer::list& inbl, Args&&... args) & {
    Op::exec_impl(method.cls.data(), method.name.data(), inbl, std::forward<Args>(args)...);
    return *this;
  }
