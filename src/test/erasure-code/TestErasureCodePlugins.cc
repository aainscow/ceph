...
TEST_P(PluginTest, Decode_SingleErasure) {
  // Test erasure code plugin can correctly decode a randomly selected
  // missing shard.
  //
  initialize();
  shard_id_set want_to_encode;
  ...
  shard_id_map<bufferlist> encoded(get_k_plus_m());
  erasure_code->encode(want_to_encode, old_bl, &encoded);

  random_device rand;
  mt19937 gen(rand());
  uniform_int_distribution<> chunk_range(0, get_k_plus_m()-1);
  shard_id_t random_chunk(chunk_range(gen));

  bufferptr erased_shard = buffer::create_aligned(chunk_size, 4096);
  encoded[random_chunk].begin().copy_deep(chunk_size, erased_shard);
  encoded.erase(random_chunk);

  bufferptr tmp = buffer::create_aligned(chunk_size, 4096);
  shard_id_set want;
  want.insert_range(shard_id_t(0), get_k_plus_m());
  shard_id_map<bufferptr> decode_in(get_k_plus_m());
  shard_id_map<bufferptr> decode_out(get_k_plus_m());
  for (unsigned int i = 0; i < get_k_plus_m(); i++) {
    if (i == (unsigned int)random_chunk) {
      decode_out[shard_id_t(i)] = tmp;
    } else {
      decode_in[shard_id_t(i)] = encoded[shard_id_t(i)].front();
    }
  }

  ASSERT_EQ(0, erasure_code->decode_chunks(want, decode_in, decode_out));
  for (int i = 0; i < chunk_size; i++) {
    ASSERT_EQ(decode_out[random_chunk].c_str()[i], erased_shard.c_str()[i]);
  }
}
TEST_P(PluginTest, Decode_MultipleErasures) {
  // Test erasure code plugin can correctly decode m number of randomly selected
  // missing shards.
  //
  initialize();
  if (get_plugin() == std::string("shec")) {
    GTEST_SKIP() << "shec does not support recovering m erasures";
  }
  shard_id_set want_to_encode;
  for (shard_id_t i ; i < get_k_plus_m(); ++i) {
    want_to_encode.insert(i);
  ...
  mt19937 gen(rand());
  uniform_int_distribution<> chunk_range(0, get_k_plus_m()-1);
  std::vector<int> random_chunk(get_m());
  for (unsigned int i = 0; i < get_m(); i++) {
    int random_number(chunk_range(gen));
    while (std::find(random_chunk.begin(), random_chunk.end(), random_number) != random_chunk.end()) {
      random_number = chunk_range(gen);
    }
    random_chunk[i] = random_number;
  }

  bufferptr erased_shard[get_m()];
  for (unsigned int i = 0; i < get_m(); i++) {
    erased_shard[i] = buffer::create_aligned(chunk_size, 4096);
    encoded[shard_id_t(random_chunk[i])].begin().copy_deep(chunk_size, erased_shard[i]);
    encoded.erase(shard_id_t(random_chunk[i]));
  }

  bufferptr tmp[get_m()];
  for (unsigned int i = 0; i < get_m(); i++) {
    tmp[i] = buffer::create_aligned(chunk_size, 4096);
  }
  shard_id_set want;
  want.insert_range(shard_id_t(0), get_k_plus_m());
  shard_id_map<bufferptr> decode_in(get_k_plus_m());
  ...
  int tmp_i = 0;
  for (unsigned int i = 0; i < get_k_plus_m(); i++) {
    if (std::find(random_chunk.begin(), random_chunk.end(), i) != random_chunk.end()) {
      decode_out[shard_id_t(i)] = tmp[tmp_i];
      tmp_i++;
    } else {
      decode_in[shard_id_t(i)] = encoded[shard_id_t(i)].front();
    }
  ...
  for (unsigned int i = 0; i < get_m(); i++) {
    for (int j = 0; j < chunk_size; j++) {
      ASSERT_EQ(decode_out[shard_id_t(random_chunk[i])].c_str()[j], erased_shard[i].c_str()[j]);
    }
  }
}
TEST_P(PluginTest,EncodeChunks_MissingDataShards)
{
  // Test encode_chunks() can be called without providing
  // buffers for all data shards.
  //
  initialize();
  for (unsigned int i = 0; i < get_k(); i++) {
    shard_id_map<bufferptr> in(get_k_plus_m());
    shard_id_map<bufferptr> out(get_k_plus_m());

    // Start at 0 and skip creating higher ID data buffers
    for (unsigned int data_shard = 0; data_shard < i; data_shard++) {
      in[shard_id_t(data_shard)] = create_bufferptr();
    }
    // Create all parity buffers
    for (unsigned int parity_shard = get_k(); parity_shard < get_k_plus_m(); parity_shard++) {
      out[shard_id_t(parity_shard)] = buffer::create_aligned(chunk_size, 4096);
    }
    ASSERT_EQ(0, erasure_code->encode_chunks(in, out));
  }
  for (unsigned int i = 0; i < get_k(); i++) {
    shard_id_map<bufferptr> in(get_k_plus_m());
    shard_id_map<bufferptr> out(get_k_plus_m());

    // Start at k-1 and skip creating lower ID data buffers
    for (unsigned int data_shard = get_k()-1; data_shard > i; data_shard--) {
      in[shard_id_t(data_shard)] = create_bufferptr();
    }
    // Create all parity buffers
    for (unsigned int parity_shard = get_k(); parity_shard < get_k_plus_m(); parity_shard++) {
...
TEST_P(PluginTest,EncodeChunks_MissingParityShards)
{
  // Test encode_chunks() can be called without providing
  // buffers for all parity shards.
  //
  initialize();
  for (unsigned int i = get_k(); i < get_k_plus_m(); i++) {
    ...
    for (unsigned int data_shard = 0; data_shard < get_k(); data_shard++) {
      in[shard_id_t(data_shard)] = create_bufferptr();
    }
    // Start at k and skip creating higher ID parity buffers
    for (unsigned int parity_shard = get_k(); parity_shard < i; parity_shard++) {
      out[shard_id_t(parity_shard)] = buffer::create_aligned(chunk_size, 4096);
    }
    ASSERT_EQ(0, erasure_code->encode_chunks(in, out));
  }
  for (unsigned int i = get_k(); i < get_k_plus_m(); i++) {
    ...
    // Start at k_plus_m and skip creating lower ID parity buffers
    for (unsigned int parity_shard = get_k_plus_m()-1; parity_shard > i; parity_shard--) {
      out[shard_id_t(parity_shard)] = buffer::create_aligned(chunk_size, 4096);
    }
    ASSERT_EQ(0, erasure_code->encode_chunks(in, out));
  }
