...
TEST(ECUtil, stripe_info_t_chunk_mapping)
{
  ...
  stripe_info_t forward_sinfo1(k, chunk_size*k, m, vector<int>(0));
...
TEST(ECUtil, shard_extent_map_t_insert_ro_buffer)
{
  ...
  for (char i=0; i<44; i++) {
    ...
    bufferlist tmp;
    ref_semap.get_buffer(shard, offset, 1024, tmp, false);
    tmp.c_str()[0] = c++;
  }
  ...
  {
    bufferlist tmp;
    ref_semap.get_buffer(1, 14*1024, 1024, tmp, false);
    tmp.c_str()[0] = c++;
  }
  insert_bl.c_str()[1024] = c;
  {
    bufferlist tmp;
    ref_semap.get_buffer(1, 15*1024, 1024, tmp, false);
    tmp.c_str()[0] = c++;
  }
