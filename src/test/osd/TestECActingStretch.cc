...
// Compute zone IDs for OSDs
static int dc_of(const OSDMapRef& osdmap, int osd, int rule, int crush_bucket_barrier)
{
  return osdmap->crush->get_parent_of_type(
    osd, crush_bucket_barrier, rule);
}
...
TEST_F(TestECActingStretch, ZonePrimary_EC_FullActingSet)
{
  ...
  int dc0 = dc_of(osdmap, 0, crush_rule, pool->peering_crush_bucket_barrier);
  int dc1 = dc_of(osdmap, 3, crush_rule, pool->peering_crush_bucket_barrier);
  ASSERT_NE(dc0, dc1);
...
TEST_F(TestECActingStretch, ZonePrimary_EC_FullActingMixed)
{
  ...
  const int crush_rule = pool->crush_rule;
  int dc0 = dc_of(osdmap, 2, crush_rule, pool->peering_crush_bucket_barrier);
  int dc1 = dc_of(osdmap, 3, crush_rule, pool->peering_crush_bucket_barrier);
  ASSERT_NE(dc0, dc1);
...
TEST_F(TestECActingStretch, ZonePrimary_EC_CrushItemNoneSkipped)
{
  ...
  const int crush_rule = pool->crush_rule;
  int dc0 = dc_of(osdmap, 2, crush_rule, pool->peering_crush_bucket_barrier);
  int dc1 = dc_of(osdmap, 5, crush_rule, pool->peering_crush_bucket_barrier);
...
TEST_F(TestECActingStretch, ZonePrimary_One_Site_Degraded_One_OSD_Down)
{
  ...
  const int crush_rule = pool->crush_rule;
  int dc1 = dc_of(osdmap, 4, crush_rule, pool->peering_crush_bucket_barrier);
