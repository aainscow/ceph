...
void PGTransaction::decode_op(ObjectOperation &op, ceph::buffer::list::const_iterator &bl) {
  ...
  ceph_assert(tag == InitTag::None || tag == InitTag::Create ||
              tag == InitTag::Clone || tag == InitTag::Rename);
  if (tag == InitTag::None) {
  ...
  for (uint32_t i = 0; i < n; ++i) {
    uint8_t rawtype; 
    decode(rawtype, bl);
    op.omap_updates[i].first = static_cast<OmapUpdateType>(rawtype);
  ...
  for (uint32_t i = 0; i < nb; ++i) {
    ...
    ceph_assert(btype == BufferUpdateTag::Write || btype == BufferUpdateTag::Zero ||
                btype == BufferUpdateTag::CloneRange);
