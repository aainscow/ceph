...
struct ECClassicalOp : ECCommon::RMWPipeline::Op {
  ...
  PGTransactionUPtr move_pg_transaction() override { return std::move(t); }

  // This op type carries a PGTransaction that must be forwarded to remote
  // Zone Primaries via MOSDECZoneReplicate.
  bool needs_zone_replicate() const override { return true; }
};

// An Op subclass used on the zone primary side.
// When shared_t is null this is a roll-forward op; generate_transactions
// is a NOP and skip_transaction follows ECDummyOp semantics.
struct ECZoneReplicateBackendOp : ECClassicalOp {
  pg_shard_t reply_to;
  std::shared_ptr<PGTransaction> shared_t;
  ...
  bool skip_transaction(
    ...
    if (!shared_t) {
      return !pending_roll_forward.erase(shard);  // ECDummyOp behaviour
    }
    return ECClassicalOp::skip_transaction(pending_roll_forward, shard, transaction);
  }

  // Zone Primaries never re-fan-out to further zones, so move_pg_transaction
  // is never called on this op type.
  PGTransactionUPtr move_pg_transaction() final { return nullptr; }
};
...
void ECBackend::handle_zone_replicate(
  ...
  op->shared_t     = std::move(wire_op.t);
  op->reply_to     = from;
  op->client_op    = msg;
  op->trace        = trace;
  op->pipeline     = &rmw_pipeline;

  // shared_t is nullptr for a roll-forward op — skip transaction-dependent
  // setup.  generate_transactions() is a NOP in that case.
  if (op->shared_t) {
    // Populate obc_map since it isn't passed across zones
    for (auto &[hoid, obj_op] : op->shared_t->op_map) {
      ...
      if (r == 0) {
        ...
        op->shared_t->obc_map[hoid] = obc;
      } else if (r == -ENOENT) {
        // Object doesn't exist yet — new write, use an empty OBC
        auto obc = std::make_shared<ObjectContext>();
        obc->obs.oi.soid = hoid;
        obc->obs.exists = false;
        op->shared_t->obc_map[hoid] = obc;
      } else {
        // Stall on pending_commits and eventually re-peer
        dout(10) << __func__ << " local store error " << r
                  << " reading attrs/stat for " << hoid
                  << " tid=" << op->tid
                  << " — aborting zone-replicate op" << dendl;
        return;
      }
    }
