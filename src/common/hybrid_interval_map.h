...
public:
  ...
  void insert(K offset, K length, V&& value)
  {
    ...
    if (is_single()) {
      ...
      // Check if we can merge with existing single interval
      if (offset == single.offset + single.length &&
          splitter.can_merge(single.value, value)) {
        // Append and merge
        single.value = splitter.merge(std::move(single.value), std::move(value));
        single.length += length;
        return;
      }
      if (offset + length == single.offset &&
          splitter.can_merge(value, single.value)) {
        // Prepend and merge
