...
class bitset_set {
  enum {
    BITS_PER_UINT64 = 64, WORDS = NumBitsV / 64, MAX = NumBitsV
  };

  // N must be a multiple of 64 and greater than 0.
  // There is no real limit to size.
  static_assert(NumBitsV % BITS_PER_UINT64 == 0);
  static_assert(NumBitsV > 0);
 ...
 public:
  ...
  void insert(const KeyT k) {
    ...
    words[int(k) / BITS_PER_UINT64] |= 1ULL << (int(k) % BITS_PER_UINT64);
  }
  ...
  void insert_range(const KeyT start, int length) {
    ...
    if (start_word == end_word) {
      words[start_word] |=
        ((1ULL << length) - 1) << (int(start) % BITS_PER_UINT64);
    } else {
      words[start_word] |= -1ULL << (int(start) % BITS_PER_UINT64);
      while (++start_word < end_word) {
        words[start_word] = -1ULL;
      }
      words[end_word] |=
        (1ULL << ((int(start) + length) % BITS_PER_UINT64)) - 1;
    }
  }
  ...
  bool contains(KeyT k) const {
    ceph_assert(int(k) < MAX);
    return (words[int(k) / BITS_PER_UINT64]
      & 1ULL << (int(k) % BITS_PER_UINT64));
  }
