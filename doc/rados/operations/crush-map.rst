...
Create the following CRUSH rule for a stretch cluster spread across geographically separated data centers::
   ...
   rule stretch_rule {
      ...
      step chooseleaf firstn 2 type {host-failure-domain}
      step emit
   }
...
2. **Items per zone**: Each zone bucket must contain at least 2 items of type
   ``osd-failure-domain``.
...
.. prompt:: bash $
   ...
   [{osd-failure-domain}] [{zones}] [--force] [{class}]
